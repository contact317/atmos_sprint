// src/pages/Requirements/RequirementUpdate.jsx
import React, { useState } from "react";
import "./RequirementUpdate.css";
import { X, Plus, Trash2 } from "lucide-react";
import { updateRequirement } from "../../api/requirementApi";

export default function RequirementUpdate({ data = {}, keyProp, onClose }) {
  ...
  // Attachments
  const addAttachment = () => {
    change("attachments", [...form.attachments, { name: "", url: "" }]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateRequirement(keyProp, payload);

      setSaving(false);
      onClose(true);
    } catch (err) {
      console.error("Requirement update failed:", err);
      setSaving(false);
      onClose(false);
    }
  };
